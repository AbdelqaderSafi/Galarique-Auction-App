...
**Built:** `auth` (full — see above), `user` (service only), `database` (`@Global`), `mail` (Brevo), `uploads` (ImageKit), `whatsapp` (`@Global`, Baileys), `seller-verification` (WhatsApp OTP → SELLER), `categories` (GET public — fixed enum list), `auctions` (unified wizard — creates Object + Auction in one `POST`; draft/submit/edit/delete/cancel; admin review queue + approve/reject; public browse/detail; approve/reject emails). **The standalone `objects` module was removed** — object CRUD now lives entirely inside `auctions`.
